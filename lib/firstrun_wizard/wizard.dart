...
import 'steps/step_three_provider.dart';
import 'steps/step_two.dart';
import 'steps/step_two_provider.dart';
import 'widgets/action_bar.dart';
import 'widgets/steps_overview.dart';
import 'widgets/steps_progress_indicator.dart';

class FirstRunWizard extends StatelessWidget {
  const FirstRunWizard._({Key? key}) : super(key: key);

  static Provider provider({Key? key}) {
    return Provider<ProviderExamplePageProvider>(
      create: (_) => ProviderExamplePageProvider(),
      dispose: (_, provider) => provider.dispose(),
      child: FirstRunWizard._(
        key: key,
      ),
    );
  }
